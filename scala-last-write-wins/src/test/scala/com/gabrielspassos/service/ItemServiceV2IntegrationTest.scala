...
class ItemServiceV2IntegrationTest @Autowired()(private val itemService: ItemServiceV2) extends BaseIntegrationTest {
  ...
  def shouldUpsertSuccessfully(): Unit = {
    ...
    val f1 = Future(itemService.save(older))
    val f2 = Future(itemService.save(newer))
